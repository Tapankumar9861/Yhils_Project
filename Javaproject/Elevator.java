...
import java.util.*;
//An elevator goes up , it continues to go up until there are no *dropoffs or *pickuprequests in that direction
...
//keeps track of people waiting K(starting floor) : V(List of All the destinationfloor from that floor)
private Map<Integer, List<Integer>> requestedPathsMap;
//Once the people at a given floor have boarded the elevator,
//*The currentFloorDestinations array -> (keeps track of the floors the elevatorwill visit
//by setting the value at the appropriate index to true)
//Your job is to implement the
//processFloor(),
//callElevator()
//and moveElevator() functions
//We also have a main function that provides an easy way to manually test yourelevator implementation.
...
currentDirection = Direction.UP;//Assumption the lift is on ground floorinitially
do {
System.out.println("--------");
...
if (requestedPathsMap.containsKey(start))//if already START is in map, add thedestination in the list
...
if (currentFloorDestinations[floor])//Deboarding if any people who reached thisdestination floor
System.out.println("UN-BOARDING at Floor : " + floor);
if (requestedPathsMap.containsKey(floor)) {//Onboarding people and theirdestination
...
requestedPathsMap.remove(floor);//removing the entry from map as we havemarked all the destination
}
currentFloorDestinations[floor] = false;//Marked false as we are just arrived inthe current floor
moveElevator();
}
//TO DO #3
...
requestedPathsMap.isEmpty()) {//STOPPING THE ELEVATOR :checking destinations are reachedand request list is empty
currentDirection = Direction.IDLE;//this will break the while loop in ourinitial start() method
return;
} else if (isInvalidFloor(currentFloor + 1)) {//SWITCH TO DOWN direction whenreached top floor
currentDirection = Direction.DOWN;
} else if (isInvalidFloor(currentFloor - 1)) {//SWITCH TO UP direction whenreached bottom floor
